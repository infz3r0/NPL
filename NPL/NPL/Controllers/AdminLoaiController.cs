        ...
        public ActionResult Delete(FormCollection form, int? page)
        {
            ...
            if (loai != null)
            {
                if (ModelState.IsValid)
                {
                    ...
                    {
                        int countMonAn = data.MonAns.Count(i => i.IDLoai == id);
                        ViewBag.IsError = true;
                        if (ex.Message.Contains("The DELETE statement conflicted with the REFERENCE constraint"))
                        {
                            ViewBag.ErrorBody = string.Format("Không thể xóa loại [{0}] do có {1} món ăn trong loại này.", loai.TenLoai, countMonAn);
                        }
                        else
                        {
